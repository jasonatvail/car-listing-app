...
- `GET /api/listings` - Get car listings with optional filters
  - Query params: `limit`, `offset`, `q` (text search), `vin`, `listing_id`, `make_id`, `model_id`, `min_year`, `max_year`, `min_price`, `max_price`, `min_odometer`, `max_odometer`, `drive`, `transmission`, `with_coords`
